...
final class LayoutFactory {
    ...
    LayoutParams getLayoutParams(String id) {
        LayoutParams params = _layoutParamMap.get(id);
        if (params == null) {
            params = new LayoutParams(
                    Config.getInt(id + ".space.leading"),
                    Config.getInt(id + ".space.intra"),
                    Config.getInt(id + ".space.trailing")
                    );
    ...
    final static LayoutCreator HLC = new LayoutCreator() {
        AbstractLayout newInstance() { return new HorizontalLayout(); }
    };
    
    final static LayoutCreator VLC = new LayoutCreator() {
        AbstractLayout newInstance() { return new VerticalLayout(); }
    };
    
    final static LayoutCreator VCLC = new LayoutCreator() {
        AbstractLayout newInstance() { return new VerticalCenteredLayout(); }
    };
    
    private static
    ILayout getLayout(String id, LayoutCreator creator) {
        LayoutParams params = getLayoutParams("layout." + id);
        String cacheId = makeCacheId("v!" + id, params.lead, params.intra, params.trail);
        ILayout layout;
